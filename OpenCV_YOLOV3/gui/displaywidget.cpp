...
DisplayWidget::DisplayWidget(QWidget *parent) : QWidget(parent)
{
    ...
    pYOLOChkBox=new QCheckBox("Using YOLO",this);
    horizontalLayout2->addWidget(sourceSelector);
    horizontalLayout2->addWidget(pYOLOChkBox);
    ...
    image_viewer_->connect(camera_,SIGNAL(cameraStopped()),SLOT(set_image_white()));
    ...
    QObject::connect(StopButton,SIGNAL(clicked()),camera_, SLOT(stopped()));
    QObject::connect(cameraComboBox, SIGNAL(currentIndexChanged(int)),camera_, SLOT(cameraIndexSlot(int)));
    QObject::connect(fileSelector, SIGNAL(clicked()),this,SLOT(openFileDialog()));
    QObject::connect(pYOLOChkBox,SIGNAL(clicked()),this,SLOT(clickYOLO()));
    QObject::connect(sourceSelector, SIGNAL(toggled(bool)),camera_, SLOT(usingVideoCameraSlot(bool)));
...
void DisplayWidget::clickYOLO()
{
    faceDector_->SetYOLO(pYOLOChkBox->isChecked());
}
